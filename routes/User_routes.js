...
router.post('/getAllStudentsByYear',async(req,res)=>{
    const {year} = req.body;
    try{
...
router.post('/addUser',authorization,async(req,res)=>{
    const {name,email,mobile,registration_no,password,year} = req.body;
    if(!name || !email || !mobile || !registration_no || !password || !year )
    {
    ...
    try{
        ...
        if(user){
            ...
            if(isMatch){
                 ...
                 const accessToken = await jwt.sign({userId : user.id,isAdmin: user.isAdmin,name: user.name,year: user.year},secretKey);
...
router.get('/rootUser',authorization,async(req,res)=>{
    ...
    const year = req.year;
    const resp = await User.findOne({_id:userId,year:year},{password:0,_id:0});
    // console.log(resp)
    res.send(resp);
})
...
router.post('/updateUser',authorization,async(req,res)=>{
    ...
    try{
        const resp = await User.updateOne(
            {_id: user.userId},
            {name:user.name,email:user.email,mobile:user.mobile,registration_no:user.registration_no,year:user.year},
        )
        if(!resp){
